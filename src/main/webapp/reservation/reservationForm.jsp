			...
			<div class="container">
				<div class="center">
					...
					<div class="row category">
						<div class="col-sm-3 custom-col">영화</div>
						<div class="col-sm-3 custom-col">극장</div>
						<div class="col-sm-2 custom-col">날짜</div>
						<div class="col-sm-4 custom-col">시간</div>
					</div>
					<form action="selectSeat.jsp" method="post">
						<div class="row">
							<div class="col-sm-3 block movie width_100">
								<div class="list-group overflow-auto" id="movieList"
									style="height: 500px;">
									<!-- 여기 영화 리스트 출력 할 거임 -->
									<%
									for (MovieInfoDto movie : movieList) {
									%>
									<a href="#" value="<%=movie.getMovieId()%>"
										class="list-group-item list-group-item-action a1"><%=movie.getMovieTitle()%></a>
									<%
									}
									%>
								</div>
							...
							<div class="col-sm-3 block">
								<div class="row">
									<div class="col-sm-6">
										<div class="list-group overflow-auto" id="movieRegion"
											style="height: 300px;">
											<!-- 여기 지역 출력 할거임 -->
											<%
											for (TheaterDto theater : theaterRegionList) {
											%>
											<a href="#" value="<%=theater.getTheater_id()%>"
												class="list-group-item list-group-item-action small a2"><%=theater.getRegion()%></a>
											<%
											}
											%>
										</div>
									...
									<div class="col-sm-6">
										<div class="list-group overflow-auto" id="movieBranch"
											style="height: 500px;">
											<!-- 여기 지점 출력 할거임 -->
											<%
											...
											%>
											<a href="#" value="<%=theater.getTheater_id()%>"
												class="list-group-item list-group-item-action a3"><%=theater.getBranch()%></a>
											<%
											}
											%>
										</div>
							...
							<div class="col-sm-2 block day width_100">
								<div class="list-group overflow-auto" id="screeningDate"
									style="height: 500px;">
									<!-- 여기 날짜 출력 할거임 -->
									<%
									for (ScreeningDto screen : screenDateList) {
									%>
									<a href="#" value="<%=screen.getScreeningDate()%>"
										class="list-group-item list-group-item-action a4"><%=sdf.format(screen.getScreeningDate())%></a>
									<%
									}
									%>
								</div>
							</div>
							<div class="col-sm-4 block time">
								<!-- 네 번째 div 내용 -->
								<div class="list-group overflow-auto" id="screeningTime"
						...
						<br>
						<div class="row">
							<div class="col rounded-1 information"
								style="background-color: black; width: 100%;">
								<div class="row row-cols-4">
									<input type="hidden" value="" name="movieId" id="movieId">
									<input type="hidden" value="" name="theaterId" id="theaterId">
									<input type="hidden" value="" name="screeningInfoId"
										id="screeningInfoId"> <input type="hidden" value=""
										name="branch" id="branch"> <input type="hidden"
										value="" name="screeningDate" id="screeningDateVal"> <input
										type="hidden" value="" name="screeningData" id="screeningData">

									<div class="col-sm d-flex justify-content-center">
										<table class="table-borderless" id="writeMovie">
											<tr>
												<td rowspan="3" style="vertical-align: middle;" class="img">
													<img src="" id="imgg">
												</td>
									...
									<div class="col-sm d-flex justify-content-center">
										<button type="submit" class="btn btn-dark" id="go">좌석선택</button>
									</div>
								</div>
							</div>
						</div>
		...
		;
							...
							$(".a4").click(
									function(event) {
										...
										screeningDateFormat = ($(this).text()
												.trim()).substr(-3, 4); // 상영날짜 텍스트 가져오기

										// 함수 호출 및 값 전달
										getScreeningTime(movieId, branch,
												screeningDate);
									});
							// 상영 시간 클릭 시
							$("#screeningTime")
							...
							// 상영정보 가져오기
							function getScreeningAndTheaterScreen(movieId,
									branch) {
							...
							// 상영 시간 정보 가져오기
							function getScreeningTime(movieId, branch,
									screeningDate) {
								console.log("현재까지 클릭된 영화와 지역과 날짜: " + movieId
										+ branch + screeningDate);
											...
											success : function(data) {
												...
												if (data.length === 0) {
													...
													screeningTimeContainer
															.append("<span id='a4hidden'>영화, 지역, 지점을 먼저 선택해 주세요.</span>");
												} else {
															...
															.each(
																	...
																	function(
																		...
																		s += "<span><strong>"
																				...
																				+ screenInfo
																				+ "</strong></span><br>";
																		s += "<button type='button' screenData='"+ screenName +" " + screenInfo +"' class='btn btn-light time' id='time' value='"+screeningInfoId+"'>"
																				+ screeningTime
																				+ "</button>";
																		s += "</div>";
